...
import os
TOKEN = os.getenv("TOKEN", "8660013437:AAGRBtmcEp1J9HBo41pSzkK6xexv63B8ESI")
...
ROLES = {
    ...
    "Оборотень": {
        "team": "werewolf", "emoji": "🐺",
        "desc": "Нейтрал. Если Шериф проверит — станет Офицером (мирным). Если убьёт мафия — станет её членом."
    },
    "Поджигатель": {
        "team": "arsonist", "emoji": "🔥",
    ...
    "Сержант": {
        "team": "citizen", "emoji": "🎖",
        "desc": "Помощник Шерифа. Каждую ночь выбирает РОВНО ОДНО из двух: проголосовать за УБИЙСТВО игрока (тот умрёт) или ПРОВЕРИТЬ игрока (узнать мафия/не мафия). Оба действия за одну ночь сделать нельзя."
    },
    "Офицер": {
        "team": "citizen", "emoji": "🪖",
        "desc": "Мирный. Появляется когда Шериф проверяет Оборотня."
    },
    "Лох ипани": {
        "team": "citizen", "emoji": "🤡",
...
class GameState:
    ...
    def team_count(self, team):
        return sum(1 for p in self.players.values()
                   if p["alive"] and ROLES[p["role"]]["team"] == team)
    ...
    def mafia_ids(self):
        return [uid for uid, p in self.players.items()
                if ROLES[p["role"]]["team"] == "mafia" and p["alive"]]

    def dead_players(self):
        return {uid: p for uid, p in self.players.items() if not p["alive"]}
...
def check_win(game):
    ...
    neutrals = {t: bool(game.get_role(r)) for r, t in [
        ("Маньяк","maniac"), ("Серийный Убийца","serial"),
        ("Оборотень","werewolf"), ("Поджигатель","arsonist"), ("Маг","mage"),
        ("АЛЛАХУ АКБАР","akbar"),
    ]}
    n_count = sum(1 for v in neutrals.values() if v)

    # Поджигатель победил?
    if neutrals["arsonist"] and game.arsonist_fired:
        return "arsonist"

    # Одиночки
    if neutrals["maniac"] and mafia == 0 and citizens == 0 and n_count == 1:
        return "maniac"
    if neutrals["maniac"] and total == 1:
        return "maniac"
    if neutrals["serial"] and mafia == 0 and citizens == 0 and n_count == 1:
        return "serial"
    if neutrals["serial"] and total == 1:
        return "serial"
    if neutrals["mage"] and mafia == 0 and citizens == 0 and n_count == 1:
        return "mage"

    # Мирные победили?
    if mafia == 0 and not any(neutrals[t] for t in ["maniac","serial","arsonist","mage","akbar"]):
        return "citizen"

    # Мафия победила?
    if mafia >= citizens + n_count:
        return "mafia"
...
def announce_win(game, winner):
    texts = {
        "citizen":  ("☀️", "Мирные победили! Город очищен!"),
        "mafia":    ("🌑", "Мафия победила! Город в руках преступников!"),
        "maniac":   ("🔪", "Маньяк победил! Все мертвы..."),
        "serial":   ("☠️", "Серийный убийца победил!"),
        "arsonist": ("🔥", "Поджигатель победил! Всё сгорело!"),
        "mage":     ("🧙", "Маг победил! Магия сильнее всех!"),
        "akbar":    ("💥", "АЛЛАХУ АКБАР победил! Взрыв унёс жертву!"),
    }
    em, txt = texts.get(winner, ("❓", "Игра окончена"))
    reveal = "\n".join(
        f"{ROLES[p['role']]['emoji']} {p['name']} — {p['role']} {'✅' if p['alive'] else '💀'}"
        for p in game.players.values()
    )
    send(game.chat_id, f"{em} <b>{txt}</b>\n\n<b>Роли:</b>\n{reveal}")
...
def build_roles_text():
    groups = [
        ...
        ("🟢 МИРНЫЕ", ["Шериф","Доктор","Диди","Телохранитель","Хакер","Гадалка","Герой","Бомж","Сержант","Офицер","Лох ипани"]),
    ]
    text = "📖 <b>Все роли:</b>\n"
...
def cb_akbar(call):
    ...
    if tid == "skip":
        ...
        return
    if aid in g.blocked:
        pm(aid, "💋 Диди заблокировала тебя — взрыв отменён!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
    tid = int(tid)
    g.night_actions["АЛЛАХУ АКБАР"] = tid
    g.akbar_won = True   # победа засчитается при разрешении
...
def cb_sheriff(call):
    ...
    if g.phase != "night": return
    if sid in g.blocked:
        pm(sid, "💋 Диди заблокировала тебя — проверка отменена!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
    if role == "Оборотень":
        g.players[tid]["role"] = "Офицер"
        pm(sid, f"🔍 Проверил {bold(g.players[tid]['name'])}: 🐺 Оборотень → теперь стал 🪖 Офицером!")
        pm(tid, "🪖 <b>Шериф проверил тебя! Ты стал Офицером (мирный)!</b>")
    # Маг — выживает и мстит
    elif role == "Маг" and not g.mage_triggered:
        g.mage_triggered = True
        g.mage_revenge = sid
        pm(sid, f"🧙 Маг отразил проверку! Он жив.")
        pm(tid, f"🧙 <b>Маг</b>: Шериф проверял тебя! Ты выжил!\nОтомстить Шерифу?")
        threading.Timer(0.5, lambda: send_mage_revenge(g, tid, sid)).start()
    else:
        pm(sid, f"🔍 {bold(g.players[tid]['name'])}: {r['emoji']} {role} ({role_team_str(role)})")
    bot.answer_callback_query(call.id, "Проверено!")
...
def cb_doctor(call):
    ...
    if g.phase != "night": return
    if did in g.blocked:
        pm(did, "💋 Диди заблокировала тебя — лечение отменено!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
...
def cb_fortune(call):
    ...
    if g.phase != "night": return
    if gid in g.blocked:
        pm(gid, "💋 Диди заблокировала тебя — гадание отменено!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
    real_role = g.players[tid]["role"]
    all_roles = [r for r in ROLES.keys() if r != real_role and r != "Офицер"]
...
def cb_sgt_kill(call):
    ...
    if g.sgt_action != "kill":
        bot.answer_callback_query(call.id, "❌ Действие недействительно!"); return
    if sid in g.blocked:
        pm(sid, "💋 Диди заблокировала тебя — голос отменён!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
    tname = g.players[tid]["name"]
...
def cb_maniac(call):
    ...
    if g.phase != "night": return
    if mid in g.blocked:
        pm(mid, "💋 Диди заблокировала тебя — убийство отменено!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
...
def cb_serial(call):
    ...
    if g.phase != "night": return
    if sid in g.blocked:
        pm(sid, "💋 Диди заблокировала тебя — убийство отменено!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
...
def cb_ars_mine(call):
    ...
    if g.phase != "night": return
    if aid in g.blocked:
        pm(aid, "💋 Диди заблокировала тебя — минирование отменено!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
    g.mined.add(tid)
    tname = g.players[tid]["name"]
...
def cb_ars_fire(call):
    ...
    if len(g.mined) < 3:
        ...
        return
    if aid in g.blocked:
        pm(aid, "💋 Диди заблокировала тебя — поджог отменён!")
        bot.answer_callback_query(call.id, "Заблокирован!"); return
...
def resolve_night(game):
    ...
    def try_kill(target_id, killer_team="?"):
        ...
        if not game.players[target_id]["alive"]: return
    ...
    if akbar_target:
        ...
        try_kill(akbar_target, "💥 АЛЛАХУ АКБАР")
    ...
    # Победа поджигателя
    if game.arsonist_fired:
        announce_win(game, "arsonist")
        return

    # Победа АЛЛАХУ АКБАР
    if game.akbar_won:
        announce_win(game, "akbar")
        return
